...
function shift(value: number, digits: number): number {
  ...
  if (s.includes("e") || s.includes("E")) return value * 10 ** digits;
...
export function fromMinor(minor: number, digits = 2): number {
  if (!Number.isFinite(minor)) return Number.NaN;
  return shift(minor, -digits);
}

/** Round to a fixed number of decimals. */
export function roundToDigits(value: number, digits = 2): number {
 ...
 */
export function roundToStep(value: number, step: number, digits = 2): number {
  if (!Number.isFinite(value)) return value;
  if (!(step > 0)) return roundToDigits(value, digits);
  const stepMinor = toMinor(step, digits);
  if (stepMinor <= 0) return roundToDigits(value, digits);
...
export function getSeparators(locale: string = DEFAULT_LOCALE): NumberSeparators {
  try {
 ...
 */
export function currencyAffix(options: MoneyFormatOptions = {}): CurrencyAffix {
  const {
  ...
  try {
    ...
    if (currencyIndex < 0) return { text: currency, side: "before" };
    return {
 ...
 */
function normaliseDecimal(digitsAndMarks: string): string {
  ...
  if (dot >= 0 && comma >= 0) {
    ...
    if (only >= 0) {
      ...
      if (!grouped) decimalIndex = only;
    }
  }

  const onlyDigits = (s: string) => s.replace(/\D/g, "");
  if (decimalIndex < 0) return onlyDigits(digitsAndMarks);
...
export function parseMoney(
  ...
  const negative =
    /^\s*\(.*\)\s*$/.test(trimmed) || LEADING_MINUS.test(trimmed) || TRAILING_MINUS.test(trimmed);
