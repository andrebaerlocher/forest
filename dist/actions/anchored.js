...
export function anchored(node, options) {
    let { anchor, placement = "bottom-start", offset = 6, flip = true, shift = true, enabled = true, matchWidth = false, } = options;
    let frame = null;
    let resizeObserver = null;
    let listening = false;
    function resolveFlip(side, anchorRect, surfaceRect, bounds) {
        if (!flip)
            return side;
        if (side === "top" || side === "bottom") {
            ...
            if (side === "top" && spaceAbove < needed && spaceBelow > spaceAbove)
                return "bottom";
        ...
        if (side === "left" && spaceLeft < needed && spaceRight > spaceLeft)
            return "right";
    ...
    function reposition() {
        ...
        if (side === "top" || side === "bottom") {
            ...
            if (shift) {
                left = clamp(left, bounds.left + VIEWPORT_MARGIN, bounds.right - surfaceRect.width - VIEWPORT_MARGIN);
            }
        }
        else {
            left =
            ...
            if (shift) {
                top = clamp(top, bounds.top + VIEWPORT_MARGIN, bounds.bottom - surfaceRect.height - VIEWPORT_MARGIN);
            }
        }
