...
>
  ...
  {#if selectedItem && !disabled}
    <button
      ...
      data-tap-target-exempt="Sits inside the text input. A centred 44px box
        would reach ~39px into the text area, so tapping the tail of what you
        typed would clear the field — worse than a small target. 40px on coarse
        pointers is the most that can be spent without stealing text taps. See
        the note above its @media (pointer: coarse) block."
    >
...
<style>
  .combobox {
    ...
    align-items: center;
    width: 100%;
    /* Fits a 360px viewport with 16px margins either side, instead of
       overflowing when 300px alone is wider than the screen. */
  ...
  /* Gated: a stuck hover on touch would leave the clear icon looking lit
