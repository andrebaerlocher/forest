...
<style>
  .menu {
    min-width: 180px;
    /* anchored.ts clamps position 8px (VIEWPORT_MARGIN) off each viewport
       edge; this caps width to match so a wide menu on a narrow phone can
       never be shifted so far its far edge still overflows. Item labels
       already wrap (no nowrap on .item-label), so no internal scroller is
       needed here. */
