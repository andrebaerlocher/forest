...
<style>
  .popover {
    ...
    /* anchored.ts clamps position 8px (VIEWPORT_MARGIN) off each viewport
       edge; this caps width to match, matching Menu's cap. Popover content
       is arbitrary (a form, a colour grid, …) and can genuinely be unable
       to shrink to fit, so it scrolls inside itself rather than overflow
       the viewport. */
